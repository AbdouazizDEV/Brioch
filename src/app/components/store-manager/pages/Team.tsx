...
// Mock Preparators for this store
const PREPARATORS = [
  { id: 'prep-1', name: 'Aminata Sall', store: 'Almadies', phone: '+221 77 123 45 67', status: 'Actif', orders: 18, avgTime: '16min', perf: 95 },
  { id: 'prep-2', name: 'Fatou Ba', store: 'Almadies', phone: '+221 77 234 56 78', status: 'Actif', orders: 15, avgTime: '18min', perf: 92 },
];
...
export default function StoreManagerTeam({ storeId }: { storeId: string }) {
  const [activeTab, setActiveTab] = useState<"drivers" | "preparators">("drivers");
  const [selectedMember, setSelectedMember] = useState<any>(null);
  
  // Filter by storeId
  const storeDrivers = DRIVERS.filter(d => d.storeId === storeId || !d.storeId);
  const storePreparators = PREPARATORS.filter(p => p.store === 'Almadies'); // Mock filter
    ...
    <div className="p-4 md:p-8 bg-[rgb(254,255,233)] min-h-[calc(100vh-80px)] space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <p className="text-sm text-gray-500">Gérez les livreurs et préparateurs de votre boutique</p>
        </div>
        <div className="flex gap-3">
           <div className="bg-white p-1 rounded-lg border border-gray-200 shadow-sm flex">
              ...
              >
                <Truck size={16} /> Livreurs ({storeDrivers.length})
              </button>
              <button 
              ...
              >
                <Utensils size={16} /> Préparateurs ({storePreparators.length})
              </button>
           </div>
                  ...
                  {storeDrivers.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                        Aucun livreur assigné à votre boutique
                      </td>
                    </tr>
                  ) : (
                      ...
                      <td className="px-6 py-4">
                         <div className="flex items-center gap-3">
                           ...
                           <div>
                             ...
                             <div className="text-xs text-gray-500">{prep.phone}</div>
                           </div>
                         </div>
                      </td>
      ...
      {/* Member Detail Panel - Same as Admin */}
      <AnimatePresence>
        {selectedMember && (
          <>
            <motion.div 
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                         ...
                         <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg text-sm font-medium">
                            ...
                            {selectedMember.store || "Brioche Dorée Almadies"}
                         </div>
                      </div>
