...
import { 
  LayoutDashboard, ShoppingBag, Truck, Users, Settings, 
  FileText, ClipboardList, X, Store
} from "lucide-react";
import { cn } from "@/lib/utils";

export function StoreManagerSidebar({ isOpen, onClose, storeName }: { isOpen: boolean, onClose: () => void, storeName: string }) {
  const location = useLocation();
  const isActive = (path: string) => location.pathname.includes(path);

  const items = [
    { icon: LayoutDashboard, label: "Tableau de bord", path: "/store-manager/dashboard" },
    { icon: ShoppingBag, label: "Commandes", path: "/store-manager/orders" },
    { icon: Truck, label: "Livraisons", path: "/store-manager/deliveries" },
    { icon: Users, label: "Équipe", path: "/store-manager/team" },
    { icon: ClipboardList, label: "Catalogue", path: "/store-manager/catalog" },
    { icon: FileText, label: "Rapports", path: "/store-manager/reports" },
    { icon: Settings, label: "Paramètres", path: "/store-manager/settings" },
  ];
      ...
      <aside className={cn(
        ...
        <div className="px-4 py-3 border-b border-gray-800 bg-gray-900/50">
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-1">
            <Store size={14} />
            <span>Boutique assignée</span>
          </div>
          <div className="font-bold text-sm text-white truncate">{storeName}</div>
        </div>
