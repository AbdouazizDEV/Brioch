...
import { Menu as MenuIcon, Bell, ChevronDown, Users, Settings, LogOut, Moon, Sun } from "lucide-react";
...
export function StoreManagerHeader({ title, onMenuClick, storeName }: { title: string, onMenuClick: () => void, storeName: string }) {
  return (
    <header className="h-20 bg-[rgb(254,255,233)] border-b border-gray-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10 shadow-sm">
      <div className="flex items-center gap-3 flex-1 min-w-0">
         ...
         <div className="flex flex-col min-w-0">
           ...
           <p className="text-xs text-gray-500 truncate hidden md:block">{storeName}</p>
         </div>
      </div>
...
function ProfileDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="w-8 h-8 rounded-full bg-bds-red text-white flex items-center justify-center font-bold">M</div>
        <div className="hidden md:block text-left">
          <p className="text-xs font-bold text-gray-900">Manager Boutique</p>
          <p className="text-[10px] text-gray-500">Almadies</p>
        </div>
          ...
          <>
            ...
            >
              <div className="p-4 border-b border-gray-100 flex items-center gap-3 bg-gray-50">
                 <div className="w-10 h-10 rounded-full bg-bds-red text-white flex items-center justify-center font-bold text-lg">M</div>
                 <div>
                   <p className="font-bold text-gray-900">Manager Boutique</p>
                   <p className="text-xs text-gray-500">Almadies</p>
                   <p className="text-xs text-gray-400">manager.almadies@briochedoree.sn</p>
                 </div>
              </div>
              <div className="p-2 space-y-1">
                <button onClick={() => { navigate('/store-manager/profile'); setIsOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                  <Users size={18} /> Mon profil
                </button>
                <button onClick={() => { navigate('/store-manager/reports'); setIsOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                  <Settings size={18} /> Mes statistiques
                </button>
                <button onClick={() => { navigate('/store-manager/settings'); setIsOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
